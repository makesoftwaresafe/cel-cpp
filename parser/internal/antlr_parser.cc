...
using ::google::api::expr::common::ReverseLookupOperator;
...
class ExprRecursionListener final : public ParseTreeListener {
 public:
  ...
  ~ExprRecursionListener() override = default;
...
absl::StatusOr<std::unique_ptr<cel::Ast>> AntlrParseImpl(
  ...
  try {
    ...
    return std::make_unique<cel::Ast>(std::move(expr), visitor.GetSourceInfo());
...
absl::StatusOr<std::unique_ptr<cel::Ast>> AntlrParserImpl::ParseImpl(
    const cel::Source& source,
    std::vector<cel::ParseIssue>* absl_nullable parse_issues) const {
  return AntlrParseImpl(source, macro_registry_, options_, parse_issues);
}

absl::StatusOr<std::unique_ptr<cel::Source>> AntlrParserImpl::PrepareSourceImpl(
...
absl::Status AntlrParserBuilderImpl::AddMacro(const cel::Macro& macro) {
...
absl::Status AntlrParserBuilderImpl::AddLibrary(cel::ParserLibrary library) {
  if (!library.id.empty()) {
...
absl::Status AntlrParserBuilderImpl::AddLibrarySubset(
...
absl::StatusOr<std::unique_ptr<cel::Parser>> AntlrParserBuilderImpl::Build() {
  using std::swap;
  ...
  return std::make_unique<AntlrParserImpl>(options_, std::move(macro_registry),
                                           std::move(library_ids));
}

std::unique_ptr<cel::ParserBuilder> AntlrParserImpl::ToBuilder() const {
  auto ins = std::make_unique<AntlrParserBuilderImpl>(options_);
