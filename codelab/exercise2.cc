...
#include "checker/type_checker_builder.h"  // IWYU pragma: keep, needed for codelab solution
...
#include "runtime/bind_proto_to_activation.h"  // IWYU pragma: keep
...
absl::StatusOr<std::unique_ptr<cel::Compiler>> MakeCelCompiler() {
  ...
  // cel::TypeCheckerBuilder class (see builder->GetCheckerBuilder()).
  //
...
absl::StatusOr<bool> CompileAndEvaluateWithContext(
  ...
  google::protobuf::Arena arena;
  // === Start Codelab ===
  // Update the activation to bind the AttributeContext using
  // cel::BindProtoToActivation (in
